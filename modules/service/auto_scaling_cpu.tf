resource "aws_appautoscaling_target" "app_scale_target" {
    ...
    resource_id        =format("service/%s/%s-%s-srv", var.cluster_name, var.cluster_name, var.service_name)
...
resource "aws_cloudwatch_metric_alarm" "cpu_utilization_high" {
  alarm_name          = format("%s-%s-srv-cpu-high", var.cluster_name, var.service_name)
...
resource "aws_appautoscaling_policy" "app_up" {
  name               = format("%s-%s-srv-cpu-scale-up", var.cluster_name, var.service_name)
...
resource "aws_cloudwatch_metric_alarm" "cpu_utilization_low" {
  alarm_name          = format("%s-%s-srv-cpu-low", var.cluster_name, var.service_name)
...
resource "aws_appautoscaling_policy" "app_down" {
  name               = format("%s-%s-srv-cpu-scale-down", var.cluster_name, var.service_name)
  service_namespace  = aws_appautoscaling_target.app_scale_target.service_namespace
  resource_id        = aws_appautoscaling_target.app_scale_target.resource_id
  scalable_dimension = aws_appautoscaling_target.app_scale_target.scalable_dimension
